...
if [ "$AUTH_MODE" = "hosted" ]; then
  pnpm exec tsx scripts/bootstrap-hosted-admin.ts
fi
...
FINGERPRINT="$(env | grep -E '^(VITE_|AUTH_MODE|BYPASS_EMAIL_VERIFICATION|POSTHOG_PUBLIC_KEY|POSTHOG_HOST|TURNSTILE_SITE_KEY|VITE_SELF_HOSTED|POSTHOG_SOURCEMAPS)' | sort | sha256sum | cut -d' ' -f1)"
